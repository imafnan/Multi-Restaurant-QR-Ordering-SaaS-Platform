...
exports.getMonthlyAnalytics = exports.getProductAnalysis = exports.updateOrderStatus = exports.clearOrders = exports.getOrders = exports.updateSettings = exports.getSettings = exports.deleteProduct = exports.updateProduct = exports.addProduct = exports.getProducts = exports.deleteCategory = exports.updateCategory = exports.addCategory = exports.getCategories = exports.getDashboardStats = void 0;
...
const getDashboardStats = async (req, res) => {
    try {
        ...
        const todayOrders = await Order_1.Order.find({
            restaurantId,
            createdAt: { $gte: startOfToday, $lte: endOfToday }
        });
        const acceptedOrdersToday = todayOrders.filter(o => o.status === 'accepted' || o.status === 'completed');
        ...
        // 3. Total Categories
        const totalCategories = await Category_1.Category.countDocuments({ restaurantId });
        // 4. Total Products
        const totalProducts = await Product_1.Product.countDocuments({ restaurantId });
        // 5. Total Visitors
...
const getCategories = async (req, res) => {
    try {
        ...
        const categories = await Category_1.Category.find(query).sort({ createdAt: -1 });
...
const updateCategory = async (req, res) => {
    try {
        const { id } = req.params;
        const { name, status } = req.body;
        const category = await Category_1.Category.findOne({ _id: id, restaurantId: req.restaurantId });
        if (!category) {
...
const deleteCategory = async (req, res) => {
    try {
        const { id } = req.params;
        const restaurantId = req.restaurantId;
        const category = await Category_1.Category.findOne({ _id: id, restaurantId });
        if (!category) {
            return res.status(404).json({ message: 'Category not found' });
        }
        ...
        let uncategorized = await Category_1.Category.findOne({
            restaurantId,
            name: 'Uncategorized'
        });
        if (!uncategorized) {
            uncategorized = await Category_1.Category.create({
                restaurantId,
                name: 'Uncategorized',
                status: 'active'
            });
        ...
        await Product_1.Product.updateMany({ restaurantId, categoryId: category._id }, { categoryId: uncategorized._id });
        // Delete the category
...
const getProducts = async (req, res) => {
    try {
        ...
        const products = await Product_1.Product.find(query).populate('categoryId', 'name').sort({ createdAt: -1 });
    ...
    try {
        ...
        if (files && files.length > 0) {
            for (const file of files) {
                const url = await storageService_1.storageService.uploadFile(file);
                images.push(url);
            }
        }
...
const updateProduct = async (req, res) => {
    try {
        ...
        const product = await Product_1.Product.findOne({ _id: id, restaurantId });
        if (!product) {
        ...
        for (const img of deletedImages) {
            await storageService_1.storageService.deleteFile(img);
        }
        // Handle new uploads
        const files = req.files;
        if (files && files.length > 0) {
...
const getSettings = async (req, res) => {
    try {
        const restaurant = await Restaurant_1.Restaurant.findById(req.restaurantId);
        if (!restaurant) {
    ...
    try {
        ...
        if (files) {
            if (files['logo'] && files['logo'][0]) {
                // Delete old logo
                if (restaurant.logo) {
                    await storageService_1.storageService.deleteFile(restaurant.logo);
                }
                const logoUrl = await storageService_1.storageService.uploadFile(files['logo'][0]);
                restaurant.logo = logoUrl;
            }
            if (files['banner'] && files['banner'][0]) {
                // Delete old banner
                if (restaurant.banner) {
                    await storageService_1.storageService.deleteFile(restaurant.banner);
                }
                const bannerUrl = await storageService_1.storageService.uploadFile(files['banner'][0]);
...
const getOrders = async (req, res) => {
    try {
        ...
        const totalCount = await Order_1.Order.countDocuments(query);
...
const clearOrders = async (req, res) => {
    try {
        ...
        await Order_1.Order.deleteMany({ restaurantId, status });
...
const updateOrderStatus = async (req, res) => {
    try {
        ...
        const order = await Order_1.Order.findOne({ _id: id, restaurantId });
        if (!order) {
        ...
        if (status === 'accepted' && oldStatus === 'pending') {
            ...
            for (const item of order.items) {
                ...
                if (product) {
                    ...
                    if (product.categoryId) {
                        const cat = await Category_1.Category.findById(product.categoryId);
                        if (cat)
                            resolvedCatName = cat.name;
                    }
                    await ProductSales_1.ProductSales.findOneAndUpdate({ restaurantId, productId: product._id, variantName: item.variantName || '' }, {
            ...
            const startOfToday = new Date();
            startOfToday.setHours(0, 0, 0, 0);
            await DailyAnalytics_1.DailyAnalytics.findOneAndUpdate({ restaurantId, date: startOfToday }, {
                $inc: {
...
const getProductAnalysis = async (req, res) => {
    try {
        ...
        const list = await ProductSales_1.ProductSales.find(query).sort({ totalSold: -1 });
...
const getMonthlyAnalytics = async (req, res) => {
    try {
        ...
        // 1. Daily Sales
        const todayRecord = await DailyAnalytics_1.DailyAnalytics.findOne({ restaurantId, date: startOfToday });
        const dailySales = todayRecord ? todayRecord.sales : 0;
        // 2. Monthly Sales (Sum of daily sales in this month)
        const monthlySum = await DailyAnalytics_1.DailyAnalytics.aggregate([
        ...
        const totalAcceptedSum = await DailyAnalytics_1.DailyAnalytics.aggregate([
            { $match: { restaurantId } },
        ...
        const bestProductAgg = await ProductSales_1.ProductSales.aggregate([
            { $match: { restaurantId } },
        ...
        const bestVariantAgg = await ProductSales_1.ProductSales.aggregate([
